...
**Where to find your keys:** Supabase Dashboard → Settings → API → Project API keys
- `Project URL` → `SUPABASE_URL`
- `anon / public` key → `SUPABASE_ANON_KEY`

> **Note:** Newer versions of the Supabase dashboard label the anon key as **"publishable key"** — they are the same key. Never use the `service_role` key in a client app.

For local development, uncomment and fill in the lines at the bottom of `gradle.properties`:
...
> **Important:** `local.env` files are **not** read by Android Gradle. Use `gradle.properties` (or `local.properties`) instead. Add the file to `.gitignore` if it contains real credentials.

These are injected at build time via `BuildConfig`. The build will fail immediately with a clear error message if `SUPABASE_ANON_KEY` is missing or blank.
...
3. Add these variables to the group:
   ...
   - `SUPABASE_ANON_KEY` = your Supabase **anon / public** key (JWT starting with `eyJ`, found at Settings → API)
